...
const KPICard: React.FC<{ label: string; value: string | number; icon: React.ReactNode; subtitle?: string; trend?: string }> = ({ label, value, icon, subtitle, trend }) => {
 return (
  <div className="bg-[#0d1322] border border-gray-800/50 shadow-sm rounded-2xl p-5 hover:border-slate-300 hover:border-gray-700 transition-all duration-200">
   <div className="flex items-start justify-between">
    ...
    <div className="p-2.5 bg-gray-800/50 bg-gray-800/60 rounded-lg">{icon}</div>
   </div>
...
const SearchInput: React.FC<{
 ...
 <div className="relative flex-1 min-w-[180px]">
  ...
  <input
   ...
   className="w-full bg-gray-800/50 border-gray-700 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/40 transition-all"
  />
 </div>
);
...
const Pagination: React.FC<{
 ...
 return (
  <div className="flex items-center justify-between px-6 py-4 border-t border-gray-800 bg-slate-50/80 bg-[#0b0f19]/30">
   ...
   <div className="flex items-center gap-1">
    <button
     ...
     className="p-1.5 rounded-xl border border-gray-800 border-gray-700 text-gray-400 text-gray-400 hover:text-white hover:bg-gray-800/50 hover:bg-gray-800 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
    >
    ...
    {getPages().map((page) => (
     <button
      ...
      : 'text-gray-400 text-gray-400 hover:text-white hover:bg-gray-800/50 hover:bg-gray-800'
      }`}
     >
      {page}
     </button>
...
const ViewToggle: React.FC<{
 ...
 return (
  <div className="flex items-center gap-1 bg-gray-800/50 bg-gray-800/50 rounded-xl p-1">
   <button
...
const ProductCard: React.FC<{
 ...
 const getStatusBadge = (status: Product['status']) => {
  switch (status) {
   case 'In Stock':
    return { label: 'Healthy', className: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' };
   case 'Low Stock':
   case 'Critical':
    return { label: 'Low Stock', className: 'text-amber-400 bg-amber-500/10 border-amber-500/20' };
   case 'Out of Stock':
    return { label: 'Out of Stock', className: 'text-rose-400 bg-rose-500/10 border-rose-500/20' };
  }
 };
 ...
 return (
  <div className="bg-[#0d1322] border border-gray-800/50 shadow-sm rounded-2xl p-4 hover:border-slate-300 hover:border-gray-700/50 transition-all duration-200 flex flex-col">
   <div className="flex items-start gap-3 mb-3">
    <div className="bg-gray-800/50 bg-gray-800/50 p-3 rounded-xl flex-shrink-0">
    ...
    <div className="flex-1 min-w-0">
     ...
     <p className="text-gray-400 text-xs">Barcode: {product.barcode || 'N/A'}</p>
    </div>
   </div>

   <div className="flex items-center gap-2 mb-3">
    <span className="bg-gray-800/50 bg-gray-800/60 text-gray-300 text-gray-300 text-xs px-2.5 py-1 rounded-lg border border-gray-700/50">
     {product.category}
    </span>
    <span className="bg-gray-800/50 bg-gray-800/60 text-gray-300 text-gray-300 text-xs px-2.5 py-1 rounded-lg border border-gray-700/50">
     {product.warehouse}
   ...
   <div className="bg-slate-50 bg-[#090d16]/80 p-3 rounded-xl grid grid-cols-3 text-center my-3">
    <div>
    ...
    <div>
     <p className="text-cyan-400 text-xs mb-1">Reserved</p>
     <p className="text-cyan-400 font-bold text-lg">0</p>
    </div>
    <div>
     <p className="text-rose-400 text-xs mb-1">Damaged</p>
    ...
    <div className="flex items-start justify-between mb-3">
     ...
     <div className="text-right">
      <p className="text-gray-400 text-[10px] uppercase tracking-wider">Inventory Value</p>
      <p className="text-[#06b6d4] font-bold text-sm">₱{inventoryValue.toLocaleString()}</p>
      <p className="text-gray-400 text-gray-400 text-[10px]">Updated: {product.updatedAt}</p>
     </div>
    </div>

    <div className="border-t border-gray-800 border-gray-800/60 pt-3 flex items-center justify-around text-gray-400">
     <button
      onClick={() => onView(product)}
      className="p-1.5 rounded-lg hover:bg-gray-800/50 hover:bg-gray-800/50 hover:text-white transition-all"
      title="View Details"
     >
...
const ProductFormModal: React.FC<{
 ...
 return (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
   <div className="bg-[#0d1322] border border-gray-800/50 shadow-sm rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
    <div className="flex items-center justify-between mb-6">
     <h2 className="text-xl font-bold text-white">{product ? 'Edit Product' : 'Add Product'}</h2>
     <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-gray-800/50 hover:bg-gray-700 text-gray-400 text-gray-400 hover:text-white transition-all">
      <X className="w-5 h-5" />
     </button>
    </div>
    <form onSubmit={handleSubmit} className="space-y-4">
     <div className="grid grid-cols-2 gap-4">
      <div>
       <label className="block text-sm font-medium mb-1.5 text-gray-300">SKU *</label>
       <input
        type="text"
        value={formData.sku}
      ...
      <div>
       ...
       <select
        ...
        className="w-full bg-gray-800/50 border-gray-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/40 appearance-none"
        required
       >
      ...
      <div>
       <label className="block text-sm font-medium mb-1.5 text-gray-300">Brand *</label>
       <select
      ...
      <div>
       <label className="block text-sm font-medium mb-1.5 text-gray-300">Warehouse *</label>
       <select
      ...
      <div>
       <label className="block text-sm font-medium mb-1.5 text-gray-300">Current Stock</label>
       <input
        type="number"
      ...
      <div>
       <label className="block text-sm font-medium mb-1.5 text-gray-300">Cost Price (₱)</label>
       <input
        type="number"
        step="0.01"
      ...
      <div>
       <label className="block text-sm font-medium mb-1.5 text-gray-300">Selling Price (₱)</label>
       <input
        type="number"
        step="0.01"
      ...
      <div>
       <label className="block text-sm font-medium mb-1.5 text-gray-300">Reorder Level</label>
       <input
        type="number"
     ...
     <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-800">
      <button type="button" onClick={onClose} className="px-5 py-2.5 border border-gray-800 border-gray-700 rounded-xl text-gray-400 text-gray-400 hover:text-white hover:bg-gray-800/50 hover:bg-gray-800 transition-all">
       Cancel
      </button>
...
const ViewProductDrawer: React.FC<{
 ...
 return (
  <div className="fixed inset-0 z-50 flex justify-end">
   ...
   <div className="bg-[#0d1322] border-l border-gray-800 border-gray-800 shadow-sm w-[480px] h-full overflow-y-auto p-6 animate-in slide-in-from-right duration-300">
    <div className="flex items-center justify-between mb-6">
    ...
    <div className="space-y-6">
     ...
     <div>
      <h3 className="text-sm font-medium text-gray-300 mb-2">Movement History</h3>
       ...
       <table className="w-full text-sm">
        <thead className="bg-gray-800/30 border-b border-gray-800">
         <tr>
          ...
          <th className="px-3 py-2 text-left text-xs text-gray-400">Ref</th>
          ...
          <th className="px-3 py-2 text-left text-xs text-gray-400">By</th>
         </tr>
        </thead>
        <tbody>
         {movements.length > 0 ? (
          movements.map((mov) => (
           <tr key={mov.id} className="border-b border-gray-800/50">
            <td className="px-3 py-2 text-gray-300">{mov.date}</td>
            <td className="px-3 py-2 text-gray-300">{mov.refNo}</td>
            <td className="px-3 py-2 text-gray-300">{mov.action}</td>
            <td className={`px-3 py-2 text-right font-medium ${mov.quantity > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
             {mov.quantity > 0 ? '+' : ''}{mov.quantity}
            </td>
            <td className="px-3 py-2 text-gray-300">{mov.performedBy}</td>
           </tr>
          ))
         ) : (
          <tr>
           <td colSpan={5} className="px-3 py-4 text-center text-gray-400">No movement records</td>
          </tr>
         )}
        </tbody>
       </table>
  ...
  <div className="flex-1 flex flex-col overflow-hidden">
   <main className="flex-1 overflow-y-auto p-6">
     ...
     <div className="max-w-7xl mx-auto">
      ...
      <p className="text-gray-400 mb-6">Configure sheet layout settings and sticker grids below.</p>

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
       {/* Left Panel - Settings */}
       <div className="lg:col-span-1 bg-[#0d1322] border border-gray-800/50 shadow-sm rounded-xl p-6 h-fit space-y-6">
        <div>
         <p className="text-gray-400 text-sm font-medium">PRODUCT INFO</p>
         <p className="text-white font-semibold mt-2">{product.name}</p>
         <p className="text-gray-400 text-sm">{product.sku}</p>
        </div>

        <div>
         <p className="text-gray-400 text-sm font-medium">LAYOUT PARAMETERS</p>
         <div className="mt-3 space-y-3">
          <div>
           <label className="block text-gray-400 text-gray-400 text-xs mb-1">STICKERS TO PRINT</label>
           <input
            type="number"
            value={stickers}
            onChange={(e) => setStickers(Number(e.target.value))}
            className="w-full bg-gray-800/50 border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-cyan-500"
           />
          </div>
          <div>
           <label className="block text-gray-400 text-gray-400 text-xs mb-1">COLUMNS IN GRID</label>
           <select
            value={columns}
            onChange={(e) => setColumns(Number(e.target.value))}
            className="w-full bg-gray-800/50 border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-cyan-500"
           >
          ...
          <div>
           <label className="block text-gray-400 text-gray-400 text-xs mb-1">Paper Size</label>
           <select
            value={paperSize}
          ...
          <div>
           <label className="block text-gray-400 text-gray-400 text-xs mb-1">Sticker Size</label>
           <select
            value={stickerSize}
          ...
          <div>
           <label className="block text-gray-400 text-gray-400 text-xs mb-1">Barcode Format</label>
           <select
            value={barcodeFormat}
        ...
        <div>
         <p className="text-gray-400 text-sm font-medium">DISPLAY OPTIONS</p>
         <div className="mt-3 space-y-2">
          <label className="flex items-center gap-2 cursor-pointer">
           <input
            ...
            className="w-4 h-4 rounded border-slate-300 border-gray-600 bg-gray-800/50 bg-gray-800 text-cyan-500 focus:ring-cyan-500"
           />
           <span className="text-gray-300 text-sm">Display Retail Price</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
           <input
            type="checkbox"
            ...
            className="w-4 h-4 rounded border-slate-300 border-gray-600 bg-gray-800/50 bg-gray-800 text-cyan-500 focus:ring-cyan-500"
           />
           <span className="text-gray-300 text-sm">Include QR Lookup Code</span>
          </label>
         </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
         ...
         <button className="w-full py-2.5 rounded-xl text-sm font-medium border border-gray-800 border-gray-700 text-gray-300 hover:bg-gray-800/50 hover:bg-gray-800 transition-all flex items-center justify-center gap-2">
       ...
       <div className="lg:col-span-3 bg-[#0d1322] border border-gray-800/50 shadow-sm rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
         <p className="text-gray-400 text-sm">Label Preview ({stickers} stickers)</p>
        </div>

        <div className={`grid gap-3`} style={{ gridTemplateColumns: `repeat(${columns}, 1fr)` }}>
         {Array.from({ length: Math.min(stickers, 30) }).map((_, idx) => (
          <div key={idx} className="bg-[#0b0f19] border border-gray-800 border-gray-700 rounded-lg p-3 text-center">
           <p className="text-white font-medium text-xs">{product.name}</p>
           <p className="text-gray-400 text-[10px]">{product.sku}</p>
           {showRetailPrice && (
  ...
  <div className="w-full max-w-7xl mx-auto p-4 md:p-6 space-y-6 bg-transparent text-white min-h-screen">
   ...
   <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
     ...
     <p className="text-sm text-gray-400">Manage your master list of products, stock thresholds, pricing, and classifications.</p>
    </div>
    <button
   ...
   <div className="bg-[#0d1322] border border-gray-800/50 shadow-sm rounded-2xl p-4 flex flex-wrap items-center gap-3">
    ...
    <button className="p-2.5 rounded-xl border border-gray-800 border-gray-700 text-gray-400 text-gray-400 hover:text-white hover:bg-gray-800/50 hover:bg-gray-800 transition-all">
    ...
    <button
     ...
     className="px-3.5 py-2.5 border border-gray-800 border-gray-700 rounded-xl text-gray-400 text-gray-400 hover:text-white hover:bg-gray-800/50 hover:bg-gray-800 transition-all text-sm"
    >
     Reset
    </button>
   </div>
    ...
    {viewMode === 'table' && (
     <div className="w-full overflow-x-auto overflow-y-hidden scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
      <table className="w-full min-w-[1200px] text-left text-sm border-collapse">
        <thead className="bg-[#0b0f19]/50 border-b border-gray-800">
         <tr>
          <th className="px-4 py-3.5 text-left text-xs font-medium uppercase tracking-wider text-gray-400">SKU / Code</th>
          <th className="px-4 py-3.5 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Product Name</th>
          <th className="px-4 py-3.5 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Category</th>
          <th className="px-4 py-3.5 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Brand</th>
          <th className="px-4 py-3.5 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Supplier</th>
          <th className="px-4 py-3.5 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Warehouse</th>
          <th className="px-4 py-3.5 text-center text-xs font-medium uppercase tracking-wider text-gray-400">Current Stock</th>
          <th className="px-4 py-3.5 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Unit</th>
          <th className="px-4 py-3.5 text-right text-xs font-medium uppercase tracking-wider text-gray-400">Cost Price</th>
          <th className="px-4 py-3.5 text-right text-xs font-medium uppercase tracking-wider text-gray-400">Selling Price</th>
          <th className="px-4 py-3.5 text-right text-xs font-medium uppercase tracking-wider text-gray-400">Reorder Level</th>
       ...
       <tbody>
        {paginatedProducts.map((product) => (
         <tr key={product.id} className="border-b border-slate-800 hover:bg-slate-800/30 transition-all">
          ...
          <td className="px-4 py-3.5 text-sm text-gray-300">{product.category}</td>
          <td className="px-4 py-3.5 text-sm text-gray-300">{product.brand}</td>
          <td className="px-4 py-3.5 text-sm text-gray-300">{product.supplier}</td>
          <td className="px-4 py-3.5 text-sm text-gray-300">{product.warehouse}</td>
          <td className="px-4 py-3.5 text-center text-sm text-white">{product.currentStock}</td>
          <td className="px-4 py-3.5 text-sm text-gray-300">{product.unit}</td>
          ...
          <td className="px-4 py-3.5">
           <div className="flex items-center justify-center gap-1">
            <button
             ...
             className="p-1.5 rounded-lg hover:bg-gray-800/50 hover:bg-gray-700 text-gray-400 text-gray-400 hover:text-white transition-all"
             title="View Details"
            >
             <Eye className="w-4 h-4" />
            </button>
            ...
            <button
             ...
             className="p-1.5 rounded-lg hover:bg-red-500/20 text-gray-400 text-gray-400 hover:text-red-400 transition-all"
             title="Delete"
            >
