...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
 const config: Record<string, { color: string; icon: React.ElementType }> = {
  ...
  Draft: { color: 'text-gray-400 bg-slate-500/10 border-slate-500/20', icon: FileText },
  ...
  <div className="w-full min-h-screen bg-[#0a0f1d] p-4 sm:p-6 lg:p-8 space-y-6 overflow-x-hidden">
      ...
      <div className="relative">
       <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
        <input
         ...
         className="pl-9 pr-4 py-2 rounded-xl bg-[#0f172a] border border-slate-800 text-sm text-slate-100 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/40 w-full sm:w-72"
        />
      </div>
      <div>
        <h1 className="text-xl font-bold text-white">Warehouse Operations Dashboard</h1>
       <p className="text-gray-400 text-gray-400 text-sm mt-0.5">{date} · {location}</p>
      </div>
     </div>
    </div>
    <div className="flex items-center gap-3">
     <button className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-800 text-sm font-medium text-gray-300 bg-[#0f172a] hover:bg-slate-800 transition-colors">
   ...
   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
    {kpiData.map((kpi, idx) => (
     <div
      key={idx}
      className="bg-[#0f172a] border border-slate-800/80 rounded-2xl p-5 hover:border-blue-300 hover:border-gray-600 transition-all group"
     >
    ...
    <div className="lg:col-span-8 bg-[#0f172a] border border-slate-800/80 rounded-2xl p-5">
     <div className="flex items-center justify-between mb-4">
      <div>
       ...
       <p className="text-gray-400 text-sm">Stock units and valuation trend across 7 months</p>
      </div>
    ...
    <div className="lg:col-span-4 bg-[#0f172a] border border-slate-800/80 rounded-2xl p-6 flex flex-col justify-between items-center h-full min-h-[340px]">
     <div className="flex items-center justify-between mb-4">
      <div>
       ...
       <p className="text-gray-400 text-sm">Distribution across the last 90 days</p>
      </div>
      <button className="text-gray-400 hover:text-gray-400 hover:text-gray-300 transition-colors">
       <MoreVertical className="w-4 h-4" />
      </button>
     </div>
     <ResponsiveContainer width="100%" height={300}>
    ...
    <div className="bg-[#0f172a] border border-slate-800/80 rounded-2xl p-5">
     <div className="flex items-center justify-between mb-4">
      <div>
       ...
       <p className="text-gray-400 text-sm">Stock In vs Stock Out this week</p>
      </div>
      <button className="text-gray-400 hover:text-gray-400 hover:text-gray-300 transition-colors">
       <MoreVertical className="w-4 h-4" />
      </button>
    ...
    <div className="bg-[#0f172a] border border-slate-800/80 rounded-2xl p-5">
     <div className="flex items-center justify-between mb-4">
      <div>
       ...
       <p className="text-gray-400 text-sm">Receiving, release and transfers</p>
      </div>
      <button className="text-gray-400 hover:text-gray-400 hover:text-gray-300 transition-colors">
       <MoreVertical className="w-4 h-4" />
      </button>
    ...
    <div className="lg:col-span-4 bg-[#0f172a] border border-slate-800/80 rounded-2xl p-5">
     <div className="flex items-center justify-between mb-4">
      <div>
       ...
       <p className="text-gray-400 text-sm">Items at or below reorder level</p>
      </div>
      <button className="text-blue-600 text-blue-400 text-sm hover:text-blue-700 hover:text-blue-300 transition-colors inline-flex items-center gap-1">
      ...
      {lowStockItems.map((item) => {
       ...
       return (
         <div key={item.id} className="p-3 rounded-xl bg-slate-900/40 border border-slate-800">
          ...
          <span className="text-xs text-gray-400 text-gray-400 font-mono shrink-0">
    ...
    <div className="lg:col-span-8 bg-[#0f172a] border border-slate-800/80 rounded-2xl p-5">
     <div className="flex items-center justify-between mb-4">
      <div>
       ...
       <p className="text-sm text-gray-400">Latest inventory ledger entries</p>
      </div>
      ...
      <table className="w-full text-sm table-fixed">
       <thead>
        <tr className="border-b border-slate-800">
         <th className="text-left py-3 text-gray-400 font-medium w-[24%]">Reference</th>
         <th className="text-left py-3 text-gray-400 font-medium w-[10%]">Type</th>
         <th className="text-left py-3 text-gray-400 font-medium w-[30%]">Product Name</th>
         <th className="text-right py-3 text-gray-400 font-medium w-[8%]">Qty</th>
         <th className="text-left py-3 text-gray-400 font-medium w-[14%]">User</th>
         <th className="text-left py-3 text-gray-400 font-medium w-[14%]">Time</th>
        </tr>
       </thead>
       <tbody>
        {transactions.map((tx) => (
         <tr key={tx.id} className="border-b border-slate-800/60 hover:bg-slate-900/40 transition-colors">
          ...
          <td className="py-3 text-gray-300 text-xs w-[10%]">{tx.type}</td>
          <td className="py-3 text-gray-300 font-medium truncate w-[30%]">{tx.product}</td>
          <td className="py-3 text-right text-white font-medium w-[8%]">{tx.qty}</td>
          <td className="py-3 text-gray-400 text-xs w-[14%]">
    ...
    <div className="bg-[#0f172a] border border-slate-800/80 rounded-2xl p-6">
     <div className="flex items-center justify-between mb-4">
      <div>
       ...
       <p className="text-gray-400 text-sm">On-time delivery and quality score</p>
      </div>
      <button className="text-blue-600 text-blue-400 text-sm hover:text-blue-700 hover:text-blue-300 transition-colors inline-flex items-center gap-1">
       View all <Eye className="w-3.5 h-3.5" />
      </button>
        ...
        <div className="flex items-center gap-4">
         <div className="flex-1 space-y-1.5">
          <div className="flex items-center justify-between text-xs">
           ...
           <span className="text-gray-300">{supplier.onTime}%</span>
          </div>
         ...
         <div className="flex-1 space-y-1.5">
          <div className="flex items-center justify-between text-xs">
           ...
           <span className="text-gray-300">{supplier.quality}%</span>
          </div>
          <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
           <div
    ...
    <div className="bg-[#0f172a] border border-slate-800/80 rounded-2xl p-6">
     <div className="flex items-center justify-between mb-1">
      <div>
       ...
       <p className="text-gray-400 text-xs">View only — model training is not available to Admin</p>
      </div>
