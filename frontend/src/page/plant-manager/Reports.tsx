...
const Reports: React.FC = () => {
  ...
  return <div className="w-full max-w-7xl mx-auto p-4 md:p-6 space-y-6 bg-[#090d16] text-slate-100 min-h-screen">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Chart title="Stock Movement Trend (30 Days)" empty={!movement.length}><ResponsiveContainer width="100%" height={200}><LineChart data={movement}><CartesianGrid strokeDasharray="3 3" stroke="#1e293b" /><XAxis dataKey="date" stroke="#64748b" tick={{ fontSize: 10 }} /><YAxis stroke="#64748b" tick={{ fontSize: 10 }} /><Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b' }} /><Legend /><Line type="monotone" dataKey="stockIn" stroke="#3b82f6" name="Stock In" strokeWidth={2} /><Line type="monotone" dataKey="stockOut" stroke="#ef4444" name="Stock Out" strokeWidth={2} /></LineChart></ResponsiveContainer></Chart>
      <Chart title="Stock In vs Stock Out" empty={!weekly.length}><ResponsiveContainer width="100%" height={200}><BarChart data={weekly}><CartesianGrid strokeDasharray="3 3" stroke="#1e293b" /><XAxis dataKey="week" stroke="#64748b" tick={{ fontSize: 10 }} /><YAxis stroke="#64748b" tick={{ fontSize: 10 }} /><Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b' }} /><Legend /><Bar dataKey="stockIn" fill="#3b82f6" name="Stock In" radius={[6, 6, 0, 0]} /><Bar dataKey="stockOut" fill="#ef4444" name="Stock Out" radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer></Chart>
...
const Donut = ({ data }: { data: Array<{ name: string; value: number; color: string }> }) => <ResponsiveContainer width="100%" height={200}><PieChart><Pie data={data} cx="50%" cy="50%" innerRadius={50} outerRadius={70} dataKey="value">{data.map((entry) => <Cell key={entry.name} fill={entry.color} stroke="#111827" strokeWidth={2} />)}</Pie><Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b' }} /><Legend /></PieChart></ResponsiveContainer>;
