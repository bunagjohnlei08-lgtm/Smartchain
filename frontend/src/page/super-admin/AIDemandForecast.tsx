...
const KPICard: React.FC<{
  ...
  <div className={`bg-[#131C2E] border border-[#1E293B] rounded-2xl p-6 shadow-xl hover:border-[#334155] transition-all duration-200 h-full flex flex-col ${className}`}>
    <div className="flex items-start justify-between flex-1">
      ...
      <div className="p-2.5 bg-[#0B1220] rounded-lg shrink-0">
        {icon}
      </div>
    </div>
    {trend && (
      <div className="mt-3 flex items-center gap-1.5 pt-2 border-t border-[#1E293B]/50">
...
const FilterSelect: React.FC<{
  ...
  <div className={`min-w-[130px] ${className}`}>
    <select
      ...
      className="w-full bg-[#0B1220] border border-[#1E293B] rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#3B82F6]/50 focus:border-[#3B82F6] transition-all appearance-none cursor-pointer"
    >
      {options.map((opt) => (
...
const AIDemandForecast: React.FC = () => {
  ...
  return (
    <PageContainer>
      <div className="space-y-6 font-sans bg-[#0B1220] min-h-screen">
        {/* Header */}
        <div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2 bg-[#131C2E] border border-[#1E293B] rounded-xl p-1">
          ...
          <div className="flex items-center gap-2">
            <button className="p-2.5 rounded-xl border border-[#1E293B] text-[#94A3B8] hover:text-white hover:bg-[#1E293B] transition-all duration-200">
            ...
            <button
              ...
              className="p-2.5 rounded-xl border border-[#1E293B] text-[#94A3B8] hover:text-white hover:bg-[#1E293B] transition-all duration-200"
            >
        ...
        {/* Main Chart */}
        <div className={`bg-[#131C2E] border border-[#1E293B] rounded-2xl p-6 shadow-xl transition-all duration-200 ${expandedChart ? 'col-span-full' : ''}`}>
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-white text-xl font-bold">Demand Forecast — Next Quarter</h3>
              <p className="text-[#94A3B8] text-sm mt-1">Historical sales vs Random Forest projection</p>
            </div>
        ...
        <div className="w-full">
          <div className="w-full bg-[#131C2E] border border-[#1E293B] rounded-2xl p-6 shadow-xl">
            {/* Top Toolbar Card */}
            <div className="bg-[#0B1220] border border-[#1E293B] p-4 rounded-xl flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
              <div className="relative flex-1 max-w-sm">
                ...
                <input
                  ...
                  className="bg-[#131C2E] border border-[#1E293B] pl-10 pr-4 py-2 text-xs rounded-xl w-full text-white focus:outline-none focus:border-blue-500"
                />
              </div>
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <FilterSelect
                  ...
                  options={statuses}
                />
                <button className="px-3.5 py-2.5 border border-[#1E293B] rounded-xl text-slate-400 hover:text-white hover:bg-[#1E293B] transition-all flex items-center gap-1.5 text-xs whitespace-nowrap">
            ...
            <div className="w-full overflow-x-auto">
                ...
                <thead className="bg-[#0B1220]/60 text-slate-400 uppercase tracking-wider text-[11px] font-semibold border-b border-[#1E293B]">
                  <tr>
                ...
                <tbody className="divide-y divide-[#1E293B]">
                  {paginatedProducts.map((product) => (
                    <tr key={product.id} className="hover:bg-[#1E293B]/40 transition">
                      <td className="py-4 px-6 whitespace-nowrap text-white font-medium">{product.name}</td>
                      <td className="py-4 px-6 whitespace-nowrap text-slate-400">{product.sku}</td>
                      <td className="py-4 px-6 whitespace-nowrap text-slate-300">{product.warehouse}</td>
                      <td className="py-4 px-6 whitespace-nowrap text-right text-slate-300">{product.historicalDemand}</td>
                      <td className="py-4 px-6 whitespace-nowrap text-right text-blue-400 font-semibold">{product.predictedDemand}</td>
                      <td className="py-4 px-6 whitespace-nowrap text-right text-slate-300">{product.currentStock}</td>
                      ...
                      <td className="py-4 px-6 whitespace-nowrap">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            ...
                            className="p-1.5 rounded-lg hover:bg-[#1E293B] text-slate-400 hover:text-white transition-all duration-200"
                          >
            ...
            <div className="flex items-center justify-between text-xs text-slate-400 pt-4 border-t border-[#1E293B]">
              <div>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-1.5 rounded-xl border border-[#1E293B] text-slate-400 hover:text-white hover:bg-[#1E293B] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full mt-6">
            ...
            <div className="bg-[#131C2E] border border-[#1E293B] rounded-2xl p-6 shadow-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {mockAIInsights.map((insight) => (
                  <div
                    key={insight.id}
                    className="bg-[#0B1220] rounded-xl p-4 border border-[#1E293B] hover:border-[#334155] transition-all duration-200"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-white font-medium text-sm">{insight.title}</p>
                        <p className="text-[#94A3B8] text-sm mt-0.5">{insight.description}</p>
                      </div>
                      {insight.action && (
                        <button className="bg-[#131C2E] border border-[#1E293B] text-slate-300 hover:text-white hover:border-blue-500 px-3 py-1.5 rounded-lg text-xs transition-all duration-200 whitespace-nowrap shrink-0">
                          {insight.action}
                        </button>
                      )}
                    </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#131C2E] border border-[#1E293B] rounded-2xl p-6 shadow-xl">
              ...
              <p className="text-[#64748B] text-xs mt-1 mb-4">Next 30-day demand ranking</p>
              <div className="space-y-3">
                {topProducts.map((product, idx) => (
                  <div key={product.id} className="bg-[#0B1220] rounded-xl p-3 border border-[#1E293B] hover:border-[#334155] transition-all duration-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[#64748B] text-xs">{product.sku}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
        ...
        <div className="bg-[#131C2E] border border-[#1E293B] rounded-2xl p-6 shadow-xl">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-[#0B1220]/50 border-b border-[#1E293B]">
                <tr>
                  <th className="px-4 py-2 text-left text-[#94A3B8] text-xs font-medium uppercase tracking-wider">Generated Date</th>
                  <th className="px-4 py-2 text-left text-[#94A3B8] text-xs font-medium uppercase tracking-wider">Generated By</th>
                  <th className="px-4 py-2 text-left text-[#94A3B8] text-xs font-medium uppercase tracking-wider">Model</th>
                  <th className="px-4 py-2 text-right text-[#94A3B8] text-xs font-medium uppercase tracking-wider">Accuracy</th>
                  <th className="px-4 py-2 text-left text-[#94A3B8] text-xs font-medium uppercase tracking-wider">Forecast Period</th>
                </tr>
              </thead>
              <tbody>
                {mockForecastHistory.map((history) => (
                  <tr key={history.id} className="border-b border-[#1E293B] hover:bg-[#1E293B]/30 transition-all duration-150">
                    <td className="px-4 py-2.5 text-white text-sm">{history.generatedDate}</td>
                    <td className="px-4 py-2.5 text-[#94A3B8] text-sm">{history.generatedBy}</td>
                    <td className="px-4 py-2.5 text-[#94A3B8] text-sm">{history.model}</td>
                    <td className="px-4 py-2.5 text-right text-white text-sm font-medium">{history.accuracy}%</td>
                    <td className="px-4 py-2.5 text-[#94A3B8] text-sm">{history.forecastPeriod}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {showDrawer && selectedProduct && (
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="bg-[#131C2E] border-l border-[#1E293B] w-[480px] h-full overflow-y-auto p-6 shadow-xl">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-white">Forecast Details</h2>
                <button onClick={() => setShowDrawer(false)} className="p-1.5 rounded-lg hover:bg-[#1E293B] text-[#94A3B8] hover:text-white transition-all duration-200">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <p className="text-[#94A3B8] text-sm">{selectedProduct.sku} · {selectedProduct.warehouse}</p>
                ...
                <div className="bg-[#0B1220] rounded-xl p-4 border border-[#1E293B]">
                  <div className="flex items-center justify-between">
                    <span className="text-[#94A3B8] text-sm">Forecast Confidence</span>
                    <span className="text-white font-semibold">{selectedProduct.confidence}%</span>
                  </div>
                  <div className="w-full h-1.5 bg-[#131C2E] rounded-full mt-1.5 overflow-hidden">
                    <div
                  ...
                  <div className="flex items-center justify-between mt-3 text-sm">
                    <span className="text-[#94A3B8]">Safety Stock</span>
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-[#1E293B]">
                  ...
                  <button className="px-4 py-2.5 border border-[#1E293B] rounded-xl text-[#94A3B8] hover:text-white hover:bg-[#1E293B] transition-all duration-200 flex items-center gap-2">
                    <Download className="w-4 h-4" /> Export
                  </button>
                </div>
              </div>
